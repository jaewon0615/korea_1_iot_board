...
/*
  ! react-cookie
  :React 에서 쿠키를 쉽게 관리할 수 있도록 도와주는 '라이브러리'
  ...
  쿠키(cookies) ,쿠키 설정함수(setCookie) 쿠기제거함수(removeCookie)를 반환 
...
export default function D_react_cookie() {
  ...
  /*
    ...
    (생성된 지점으로부터 시간을 기분으로 유효 시간 설정)
    secure : true로 설정 시 https 에서만 쿠키가 전송
    sameSite : 쿠키가 전송될 조건을 제한
    - strict 속성을 사용하여 다른 사이트에서 요청에서는 쿠키가 포함되지 않도록 제한
  */
